...
public partial class NumericPicker<TValue> : Blazorise.NumericPicker<TValue>
{
    ...
    {
        // Apply width to the wrapper while the input fills the available space.
        if ( parameters.TryGetValue<IFluentSizing>( nameof( Width ), out IFluentSizing paramWidth ) )
        {
            ...
            Dictionary<string, object> remainingParameters = parameters.ToDictionary()
                .Where( x => x.Key != nameof( Width ) )
                .ToDictionary( x => x.Key, x => x.Value );

            return base.SetParametersAsync( ParameterView.FromDictionary( remainingParameters ) );
        }
    ...
    {
        AppendWrapperUtilities( builder );
        builder.Append( numericWrapperWidth?.Style( StyleProvider ) );
    }

    private static void BuildButtonsClasses( ClassBuilder builder )
    {
        builder.Append( "tw-numeric-buttons" );
    }

    private static void BuildButtonUpClasses( ClassBuilder builder )
    {
        builder.Append( "tw-numeric-button" );
        builder.Append( "tw-numeric-button-up" );
    }

    private static void BuildButtonDownClasses( ClassBuilder builder )
    {
        builder.Append( "tw-numeric-button" );
        builder.Append( "tw-numeric-button-down" );
    }
