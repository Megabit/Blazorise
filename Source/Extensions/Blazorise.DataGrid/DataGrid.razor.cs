...
namespace Blazorise.DataGrid
{
    public abstract class BaseDataGrid<TItem> : BaseComponent
    {
        ...
        private void FilterData(IQueryable<TItem> query)
        {
            ...
            NumberOfRows = filteredData.Count;
        ...
        protected IEnumerable<TItem> DisplayData
        {
            get
            {
                // the user take care about the data, using the event
                if (NeedData != null)
                {
                    if (dirtyView)
                    {
                        if (dirtyFilter)
                        {
                            ...
                            var needDataEventArgs = new NeedDataEventArgs<TItem>(
                                CurrentPage,
                                PageSize,
                                Columns,
                                numberOfRows,
                            ...
                            NeedData(needDataEventArgs);
                            NumberOfRows = needDataEventArgs.NumberOfRows;
                            Data = needDataEventArgs.Data;
                            dirtyFilter = dirtyView = false;
                        }
                    }
                    return Data;
                }
        ...
        protected int LastPage
        {
            get
            {
                ...
                var lastPage = Math.Max((int)Math.Ceiling(NumberOfRows / (double)PageSize), 1);
        ...
        protected int NumberOfRows { get; set; } = 0;
        ...
        /// <summary>
        /// Manual paging data, the user provide a data for each page.
        /// </summary>
