import { VidstackPlayer, PlyrLayout } from "./vendors/player.js?v=1.7.3.0";

import { getRequiredElement, isString, firstNonNull } from "../Blazorise/utilities.js?v=1.7.3.0";
...
export async function initialize(dotNetAdapter, element, elementId, options) {
    ...
    instance.player.addEventListener('provider-change', (event) => {
        ...
        if (provider?.type === 'hls') {
            provider.library = '_content/Blazorise.Video/vendors/hls.js?v=1.7.3.0';
        }
        else if (provider?.type === 'dash') {
            provider.library = '_content/Blazorise.Video/vendors/dash.js?v=1.7.3.0';
        }
    });
