...
internal sealed class SvgChartRadialSeriesContent : SvgChartSeriesContentBase
{
    ...
    protected override void BuildRenderTree( RenderTreeBuilder builder )
    {
        ...
        foreach ( var segment in segments )
        {
            ...
            Context.AddPointInteractionAttributes( builder, ref sequence, point, segment.Color, Series );
            Context.RenderInitialAttributeAnimation( builder, ref sequence, "opacity", "0", "1" );
            builder.CloseElement();

            if ( !segment.Hidden )
            {
                Context.RenderDataDragHitTarget( builder, ref sequence, Series, point, segment.InteractionX, segment.InteractionY, 0 );
            }
        }
    }
        ...
        IReadOnlyList<SvgChartRenderedRadialSegment> renderedSegments )
    {
        ...
        var max = series.Type == SvgChartType.PolarArea ? Math.Max( chart.ValueMax, 1 ) : visibleValues.Count > 0 ? visibleValues.Max( x => x.Value.Value ) : 0;
        ...
        foreach ( var value in values )
        {
            ...
            var interactionRadius = series.Type switch
            {
