...
window.blazorise = {
    utils: {
        getRequiredElement: function (element, elementId) {
            if (element)
                return element;
    ...
    getElementInfo: function (element, elementId) {
        if (!element) {
    ...
    hasSelectionCapabilities: function (element) {
    ...
    setCaret: function (element, caret) {
    ...
    addClosableComponent: function (elementId, dotnetAdapter) {
        window.blazorise.closableComponents.push({ elementId: elementId, dotnetAdapter: dotnetAdapter });
    },

    findClosableComponent: function (elementId) {
    ...
    findClosableComponentIndex: function (elementId) {
        for (index = 0; index < window.blazorise.closableComponents.length; ++index) {
            if (window.blazorise.closableComponents[index].elementId === elementId)
                return index;
        }
    ...
    isClosableComponent: function (elementId) {
        for (index = 0; index < window.blazorise.closableComponents.length; ++index) {
            if (window.blazorise.closableComponents[index].elementId === elementId)
                return true;
        }
    ...
    registerClosableComponent: function (elementId, dotnetAdapter) {
        if (window.blazorise.isClosableComponent(elementId) !== true) {
            window.blazorise.addClosableComponent(elementId, dotnetAdapter);
        }
    },

    unregisterClosableComponent: function (elementId) {
        var index = window.blazorise.findClosableComponentIndex(elementId);
        if (index !== -1) {
            window.blazorise.closableComponents.splice(index, 1);
        }
    },

    tryClose: function (closable, targetElementId, isEscapeKey) {
        var request = new Promise(function (resolve, reject) {
            closable.dotnetAdapter.invokeMethodAsync('SafeToClose', targetElementId, isEscapeKey ? 'escape' : 'leave')
                .then(function (result) { resolve({ elementId: closable.elementId, dotnetAdapter: closable.dotnetAdapter, status: result === true ? 'ok' : 'cancelled' }) })
                .catch(function () { resolve({ elementId: closable.elementId, status: 'error' }) });
        });
                ...
                .then(function (response) {
                    if (response.status === 'ok') {
                        response.dotnetAdapter.invokeMethodAsync('Close', isEscapeKey ? 'escape' : 'leave')
                            ...
                            .catch(function () { window.blazorise.unregisterClosableComponent(response.elementId) });
                    }
                });
        }
    },
    focus: function (element, elementId, scrollToElement) {
    ...
    textEdit: {
        ...
        initialize: function (element, elementId, maskType, editMask) {
    ...
    numericEdit: {
        ...
        initialize: function (dotnetAdapter, element, elementId, decimals, separator, step, min, max) {
    ...
    button: {
        ...
        initialize: function (element, elementId, preventDefaultOnSubmit) {
    ...
    fileEdit: {
        initialize: function (adapter, element, elementId) {
            var nextFileId = 0;
        ...
        open: function (element, elementId) {
    ...
    breakpoint: {
        ...
        addBreakpointComponent: function (elementId, dotnetAdapter) {
            window.blazorise.breakpoint.breakpointComponents.push({ elementId: elementId, dotnetAdapter: dotnetAdapter });
        },

        findBreakpointComponentIndex: function (elementId) {
        ...
        isBreakpointComponent: function (elementId) {
            for (index = 0; index < window.blazorise.breakpoint.breakpointComponents.length; ++index) {
                if (window.blazorise.breakpoint.breakpointComponents[index].elementId === elementId)
                    return true;
            }
        ...
        registerBreakpointComponent: function (elementId, dotnetAdapter) {
        ...
        unregisterBreakpointComponent: function (elementId) {
            var index = window.blazorise.breakpoint.findBreakpointComponentIndex(elementId);
            if (index !== -1) {
                window.blazorise.breakpoint.breakpointComponents.splice(index, 1);
            }
        },

        onBreakpoint: function (dotnetAdapter, currentBreakpoint) {
...
document.addEventListener('click', function handler(evt) {
    if (window.blazorise.closableComponents && window.blazorise.closableComponents.length > 0) {
        var lastClosable = window.blazorise.closableComponents[window.blazorise.closableComponents.length - 1];
