    ...
    IAsyncDisposable
{
    ...
    private DotNetObjectReference<FileInputAdapter> dotNetObjectRef;
    ...
    {
        builder.Append( ClassProvider.FileInput() );
        builder.Append( ClassProvider.FileInputValidation( ParentValidation?.Status ?? ValidationStatus.None ) );
    ...
    {
        dotNetObjectRef ??= CreateDotNetObjectRef( new FileInputAdapter( this ) );

        await JSFileInputModule.Initialize( dotNetObjectRef, ElementRef, ElementId );
    ...
    protected override async ValueTask DisposeAsync( bool disposing )
    {
        ...
        {
            await JSFileInputModule.SafeDestroy( ElementRef, ElementId );
    ...
    {
        throw new NotImplementedException( $"{nameof( OnInternalValueChanged )} in {nameof( FileInput )} should never be called." );
    }

    /// <inheritdoc/>
    protected override Task<ParseValue<IFileEntry[]>> ParseValueFromStringAsync( string value )
    {
        throw new NotImplementedException( $"{nameof( ParseValueFromStringAsync )} in {nameof( FileInput )} should never be called." );
    }
    ...
    public ValueTask Reset()
    {
        ...
        ProgressTotal = 0;
        Progress = 0;
        return JSFileInputModule.Reset( ElementRef, ElementId );
    }
    ...
    {
        return JSFileInputModule.RemoveFile( ElementRef, ElementId, fileId );
    }
    ...
    /// <summary>
    /// Gets or sets the <see cref="IJSFileInputModule"/> instance.
    /// </summary>
    [Inject] public IJSFileInputModule JSFileInputModule { get; set; }
    ...
    /// <summary>
    /// Gets or sets the DI registered <see cref="ITextLocalizer{FileInput}"/>.
    /// </summary>
    [Inject] protected ITextLocalizer<FileInput> Localizer { get; set; }
